...
struct Rect {
    ...
    constexpr Rect intersection(const Rect& o) const {
        int32_t ix = (x > o.x) ? x : o.x;
        int32_t iy = (y > o.y) ? y : o.y;
        int32_t ir = ((x + w) < (o.x + o.w)) ? (x + w) : (o.x + o.w);
        int32_t ib = ((y + h) < (o.y + o.h)) ? (y + h) : (o.y + o.h);
...
namespace Colors {
    inline constexpr Color Transparent  = 0x00000000u;
    inline constexpr Color Black        = 0xFF000000u;
    inline constexpr Color White        = 0xFFFFFFFFu;
    inline constexpr Color DarkGray     = 0xFF1E1E1Eu;
    inline constexpr Color MidGray      = 0xFF3A3A3Au;
    inline constexpr Color LightGray    = 0xFFAAAAAu;
    inline constexpr Color Accent       = 0xFF5E81F4u;
    inline constexpr Color AccentHover  = 0xFF7C9BFFu;
    inline constexpr Color AccentActive = 0xFF3D5BD4u;
    inline constexpr Color Danger       = 0xFFEF4444u;
    inline constexpr Color Success      = 0xFF10B981u;
    inline constexpr Color Warning      = 0xFFF59E0Bu;
    inline constexpr Color Text         = 0xFFE8E8F0u;
    inline constexpr Color TextDim      = 0xFF9898B0u;
...
inline Color alpha_blend(Color src, Color dst) {
    ...
    uint32_t ia = 255u - a;
    uint32_t r  = ((src >> 16 & 0xFF) * a + (dst >> 16 & 0xFF) * ia) / 255u;
    uint32_t g  = ((src >>  8 & 0xFF) * a + (dst >>  8 & 0xFF) * ia) / 255u;
    uint32_t b  = ((src       & 0xFF) * a + (dst       & 0xFF) * ia) / 255u;
    return 0xFF000000u | (r << 16) | (g << 8) | b;
}

inline Color dim(Color c, uint8_t factor) {
    uint32_t r = ((c >> 16) & 0xFF) * factor / 255u;
    uint32_t g = ((c >>  8) & 0xFF) * factor / 255u;
    uint32_t b = ( c        & 0xFF) * factor / 255u;
    return (c & 0xFF000000u) | (r << 16) | (g << 8) | b;
}

enum class EventType : uint8_t {
    MouseMove, MousePress, MouseRelease, MouseScroll,
    KeyPress, KeyRelease, FocusGained, FocusLost, Resize, Close
};
