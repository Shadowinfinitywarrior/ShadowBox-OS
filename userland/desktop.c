...
static void draw_window(window_t *w) {
    ...
    if (w->type == WTYPE_TERMINAL) {
        ...
        draw_number(w->x + 120, w->y + 54, ((uint64_t*)w->text)[0] * 4 / 1024, 0x2C3E50); 
        draw_string(w->x + 180, w->y + 54, "MB", 0x2C3E50);
        
        draw_string(w->x + 10, w->y + 74, "Drivers Loaded:", 0x2980B9);
        draw_string(w->x + 10, w->y + 89, "- AHCI (SATA)", 0x34495E);
        draw_string(w->x + 10, w->y + 104, "- e1000 & rtl8139 (Net)", 0x34495E);
        draw_string(w->x + 10, w->y + 119, "- Universal Input Manager", 0x34495E);
        draw_string(w->x + 10, w->y + 134, "- ACPI / APIC / IOAPIC", 0x34495E);
        draw_string(w->x + 10, w->y + 149, "- VESA Framebuffer", 0x34495E);
        draw_string(w->x + 10, w->y + 164, "- ShadowBox Compositor", 0x34495E);
                    ...
                    if (!handled) {
                        for (int i = num_windows - 1; i >= 0; i--) {
                            ...
                            if (mouse_x >= w->x && mouse_x <= w->x + w->w &&
                                mouse_y >= w->y && mouse_y <= w->y + 24) {
                                
                                if (mouse_x >= w->x + w->w - 24 && mouse_x <= w->x + w->w &&
                                    mouse_y >= w->y && mouse_y <= w->y + 24) {
                                ...
                                if (temp.type == WTYPE_FILE_BRO && mouse_y >= temp.y + 59) {
                                    ...
                                    if (idx >= 0 && idx < temp.num_entries) {
                                        ...
                                        if (fd >= 0) {
                                            ...
                                            strcpy(top->current_dir, "/"); // Revert if file
